...
// Si necesitas m�s includes del motor, agr�galos aqu�.
...
void UserInterface::update()
{
  ...
  // Docking opcional (lo dejamos apagado para evitar problemas de versi�n)
...
void UserInterface::drawHierarchy_()
{
  ...
  for (auto& actorPtr : *m_actors)
  {
    ...
    // ID �nico basado en el puntero del actor (evita problemas de IDs vac�os)
    ImGui::PushID(actor);
...
void UserInterface::drawInspector_()
{
  ...
  // ---------------------------------------------------------------------
  // Transform (edici�n en tiempo real)
  // ---------------------------------------------------------------------
  ...
  if (!transform.isNull())
  {
    ...
    {
      // Si tu Transform tiene getters, puedes usarlos aqu�.
